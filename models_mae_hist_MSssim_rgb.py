...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, overall_min, overall_max, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False,criterion=MS_SSIMLoss().cuda()):
        super().__init__()
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        print(f"pred_rgb_max -> {pred.min()}, pred_rgb_min -> {pred.max()}")
        ...
        print(f"img_norm_shape-> {img_norm.shape}, pred_norm_shape -> {preds_norm.shape}" )
        # print(target.shape)
        ...
        img_norm = img_norm.reshape(batch_size, -1, height, width)
        ...
        preds_norm = preds_norm.reshape(batch_size, -1, height, width)
        ...
        print(f"Min Max -> {img_norm.min()}, {img_norm.max()}")
        print(f"Min Max -> {preds_norm.min()}, {preds_norm.max()}")

        print(f"Shape -> {img_norm.shape}, {preds_norm.shape}")
        ...
        # # ssim_loss = ssim_loss.mean(dim=-1)  # [N, L], mean loss_glcm per patch
        # print(f"SSIM_loss shape -> {ssim_loss.shape}")
        ...
        #     cv2.imwrite(f"generated_glcm_MSssim_rgb_scaled_Her2Neu/{self.i}.png", to_write)
        #     cv2.imwrite(f"generated_glcm_MSssim_rgb_scaled_Her2Neu/{self.i}_rec.png", to_write_rec)
        #     cv2.imwrite(f"generated_glcm_MSssim_rgb_scaled_Her2Neu/{self.i}_rec_original.png", to_write_rec_original)
