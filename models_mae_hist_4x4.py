...
class MaskedAutoencoderViT(nn.Module):
    ...
    def further_unpatchify(self, x):
        """
        x:  (64, 196, 3 , 16 , 16) -> (64, 196*)
        imgs: (N, 3, H, W)
        """
       
        x = einops.rearrange(x, 'b p c (h1 h) (w1 w) -> b (p h1 w1) c h w ', h1=4, w1=4)
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        # x = torch.tanh(x) 
        x = torch.clamp(x, min=-2.1179039478302, max=2.640000104904175)
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        pred_unpatch = self.further_unpatchify(pred_unpatch)
        
        target = self.custom_unpatchify(target)
        target = self.further_unpatchify(target)
        ...
        mask_new  = torch.empty((2, 3136), device='cuda')
        print("mask grad-> ", mask.requires_grad)
        for i in range(mask.shape[0]):
            for j in range(mask.shape[1]):
                for k in range(16):
                    mask_new[i][j+k] = mask[i][j]
                    
        print(mask_new.shape)
                    

        # loss = (loss * mask).sum() / mask.sum()  # mean loss on removed patches
        loss = (loss * mask_new).sum() / mask_new.sum()  # mean loss on removed patches
        ...
        mask_new = mask_new.detach()
        mask_new = mask_new.unsqueeze(-1).repeat(1, 1, 4**2 *3)  # (N, H*W, p*p*3)
        print("mask_new shape -> ", mask_new.shape)
        mask_new = self.unpatchify_new(mask_new)  # 1 is removing, 0 is keeping
        mask_new = torch.einsum('nchw->nhwc', mask_new).detach().cpu()
        ...
        im_masked = x * (1 - mask_new)

        # MAE reconstruction pasted with visible patches
        im_paste = x * (1 - mask_new) + y * mask_new
        ...
        if self.i % 100 == 0:
            ...
            cv2.imwrite(f"generated_glcm_4x4/{self.i}.png", to_write)
            cv2.imwrite(f"generated_glcm_4x4/{self.i}_rec.png", to_write_rec)
            cv2.imwrite(f"generated_glcm_4x4/{self.i}_rec_original.png", to_write_rec_original)
