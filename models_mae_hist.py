...
class MaskedAutoencoderViT(nn.Module):
    ...
    def activation_func(self, img_flat, bins_av, min, max):
        ...
        interval_length = (max - min) / self.bin_num
        kernel_width = interval_length / (30)

        
        maps = sigmoid((img_minus_bins_av + interval_length / 2) / kernel_width) \
               - sigmoid((img_minus_bins_av - interval_length / 2) / kernel_width) \
               + sigmoid((img_plus_bins_av - 2 * min + interval_length / 2) / kernel_width) \
               - sigmoid((img_plus_bins_av - 2 * min - interval_length / 2) / kernel_width) \
               + sigmoid((img_plus_bins_av - 2 * max + interval_length / 2) / kernel_width) \
               - sigmoid((img_plus_bins_av - 2 * max - interval_length / 2) / kernel_width)
        # print("maps->" , maps.shape)
        return maps
    
    
    def calc_activation_maps(self, img, min, max):
        # apply approximated shifted rect (bin_num) functions on img
        # torch.set_default_device(self.device)
        
        bins_min_max = np.linspace(min, max, self.bin_num + 1)
        ...
        maps = self.activation_func(img_flat, bins_av, min, max)  # shape = (batch_size,H*W,bin_num)
        return maps

    def calc_glcm_maps(self, img, min, max):
        # hist_row = self.calc_activation_maps(img[:,:,:-1])
        hist_row = self.calc_activation_maps(img[:,:,:,:-1], min, max)
        # hist_shift_row = self.calc_activation_maps(img[:,:,1:])
        hist_shift_row = self.calc_activation_maps(img[:,:,:,1:], min, max)
        ...
        hist_col = self.calc_activation_maps(img[:,:,:-1,:], min, max)

        # hist_shift_col = self.calc_activation_maps(img[:,1:,:])
        hist_shift_col = self.calc_activation_maps(img[:,:,1:,:], min, max)
    ...
    def calc_glcm_out_tar(self, tar, out, min, max):
        glcm_tar = self.calc_glcm_maps(tar, min, max)
        glcm_out = self.calc_glcm_maps(out, min, max)
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        pred_unpatch = torch.clamp(pred_unpatch, min=target.min(), max=target.max())
        ...
        glcm_1 = self.calc_glcm_out_tar(pred_unpatch[:,:,0,:,:], target[:,:,0,:,:], target.min().detach().cpu().numpy(), target.max().detach().cpu().numpy())
        ...
        if self.i % 100 == 0:
            ...
            cv2.imwrite(f"generated_glcm_covid_new{self.i}.png", to_write)
            cv2.imwrite(f"generated_glcm_covid_new/{self.i}_rec.png", to_write_rec)
            cv2.imwrite(f"generated_glcm_covid_new/{self.i}_rec_original.png", to_write_rec_original)
