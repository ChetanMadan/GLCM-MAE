...
def main(args):
    ...
    overall_min = torch.tensor([999999])
    overall_max = torch.tensor([-999999])
    for (_, imgs) in enumerate(data_loader_train):
        print(imgs[0].shape)
        overall_min = min(imgs[0].min(), overall_min)
        overall_max = max(imgs[0].max(), overall_max)
        
    print("overaall_min", overall_min)
    ...
    model = models_mae_all_gbcu.__dict__[args.model](norm_pix_loss=args.norm_pix_loss, overall_min=overall_min, overall_max=overall_max)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            overall_min, overall_max, model, data_loader_train,
