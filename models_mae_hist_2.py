...
class MaskedAutoencoderViT(nn.Module):
    ...
    def calc_glcm_maps(self, img):
        ...
        glcm_row = torch.bmm(hist_row.permute(0, 2, 1), hist_shift_row) /  (224 * 223 )
        ...
        glcm_col = torch.bmm(hist_col.permute(0, 2, 1), hist_shift_col) /  (224 * 223 )
        ...
        x = torch.einsum('nbxy-> bnxy', torch.stack([glcm_row, glcm_col]))
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        # target_unpatch = self.custom_unpatchify(target)
        ...
        print("loss_rgb_shape ", loss_rgb.shape)
        
        
        pred_unpatch = torch.clamp(pred_unpatch, min=-2.1179039478302, max=2.640000104904175)
        
        glcm_1 = self.calc_glcm_out_tar(pred_unpatch[:,0,:,:], imgs[:,0,:,:]) 
        ...
        # loss = 3 * (glcm_1[0] - glcm_1[1]) ** 2
        loss_glcm = (glcm_1[0] - glcm_1[1]) ** 2


        loss_glcm = einops.rearrange(loss_glcm, 'b n x y -> b (n x y)')
        loss_glcm = torch.sum(loss_glcm, axis=-1) 
        loss_glcm = torch.mean(loss_glcm)
        print("loss_glcm_shape ", loss_glcm.shape)
        ...
        if self.i % 100 == 0:
            ...
            cv2.imwrite(f"generated_glcm_full_and_RGB/{self.i}.png", to_write)
            cv2.imwrite(f"generated_glcm_full_and_RGB/{self.i}_rec.png", to_write_rec)
            cv2.imwrite(f"generated_glcm_full_and_RGB/{self.i}_rec_original.png", to_write_rec_original)
