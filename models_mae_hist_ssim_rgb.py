...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, overall_min, overall_max, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False,criterion=SSIMLoss().cuda()):
        super().__init__()
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        img_norm = img_norm.view(img_norm.size(0), -1)
        img_norm -= img_norm.min(1, keepdim=True)[0]
        img_norm /= img_norm.max(1, keepdim=True)[0]
        img_norm = img_norm.view(batch_size, -1, height, width)
        ...
        preds_norm = preds_norm.view(batch_size, -1, height, width)
        ...
        print(f"Shape -> {img_norm.shape}, {img_norm.shape}")
        ...
        #     cv2.imwrite(f"generated_glcm_ssim_rgb_scaled_Her2Neu/{self.i}.png", to_write)
        #     cv2.imwrite(f"generated_glcm_ssim_rgb_scaled_Her2Neu/{self.i}_rec.png", to_write_rec)
        #     cv2.imwrite(f"generated_glcm_ssim_rgb_scaled_Her2Neu/{self.i}_rec_original.png", to_write_rec_original)
